...
function buildFaqs(hours) {
  const hoursAnswer = hours && hours.length > 0
    ? (
      <ul style={{ margin: '0.5rem 0 0', padding: 0, listStyle: 'none' }}>
        {hours.map(h => (
          <li key={h._key || h.days} style={{ marginBottom: '0.25rem' }}>
            <strong>{h.days}:</strong> {h.time}
          </li>
        ))}
      </ul>
    )
    : "Our hours change seasonally — check our website for the most current hours.";

  const seasonAnswer = hours && hours.length > 0
    ? <>We&apos;re open for the 2026 season! {hoursAnswer}</>
    : "We're open for the 2026 season — check our website for current hours.";
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <a href="https://www.935ocean.com/" target="_blank" rel="noopener noreferrer">here</a>.
        </>
      )
    },
    {
...
export default function Faq({ hours }) {
  const [activeIndex, setActiveIndex] = useState(null);
  const faqs = buildFaqs(hours);
